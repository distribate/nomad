...
export const Icon = (props: IconProps) => {
  const [local, others] = splitProps(props, ["class", "name", "invert"])
  const iconMeta = createMemo(() => getIconMeta(local.name), [local.name]);

  const { viewBox, width, height } = iconMeta().symbol;
  const href = iconMeta().href;
