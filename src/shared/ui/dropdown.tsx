	...
	return (
		<DropdownMenuPrimitive.Portal>
			<DropdownMenuPrimitive.Content
				class={cn(
					`
					  min-w-8rem z-50 overflow-hidden rounded-lg border-none bg-neutral-800 p-1
					  text-primary shadow-md data-[expanded]:(animate-in fade-in-0 zoom-in-95)
					  data-[closed]:(animate-out fade-out-0 zoom-out-95) focus-visible:(outline-none) transition-shadow
					`,
					local.class,
				)}
				{...rest}
...
export const DropdownMenuItem = <T extends ValidComponent = "div">(
	...
	return (
		<DropdownMenuPrimitive.Item
			class={cn(
				`
				  relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-base
				  outline-none transition-colors focus:(bg-accent text-accent-foreground) data-[disabled]:(pointer-events-none opacity-50)
				`,
				local.inset && "pl-8",
				local.class,
			)}
			...
			class={cn(
				`
				  flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm
					outline-none focus:bg-accent data-expanded:bg-accent
				`,
				local.class,
			)}
			{...rest}
