...
import { $feed, $feedStatuses, loadProfiles } from "./model"
import { useAtom, useCtx } from "@reatom/npm-solid-js"

export const Index = () => {
  const ctx = useCtx();

  const [profilesIsLoading] = useAtom($feedStatuses.profilesIsLoading);
  ...
  return (
    <div>
      <Show when={!profilesIsLoading()} fallback={<div>Loading profiles...</div>}>
        {(_) => {
          const [profiles] = useAtom($feed.profiles);
            ...
            <div>
              <For each={profiles()}>
                {(profile) => {
                  const photoUrl =
                    profile.photo?.src ||
                    "data:image/svg+xml;utf8,<svg xmlns='http://www.w3.org/2000/svg' width='100' height='100'><rect width='100%' height='100%' fill='%231f2937'/></svg>";
